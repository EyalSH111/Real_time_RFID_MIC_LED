...
data2_path = os.path.join(base_folder, 'Data_2.mat')
data1_path = os.path.join(base_folder, 'Data_1.mat')
...
def read_data(ser, stop_event, data, time_vector, start_time):
...
def main():
    ...
    # Open serial port with context manager for safe access
    with serial.Serial('COM7', 9600, timeout=1) as ser:
        time.sleep(2)

        # Timer and data threads
        timer_thread = threading.Thread(target=start_timer, args=(root, progress, label, duration, stop_event))
        data_thread = threading.Thread(target=read_data, args=(ser, stop_event, data, time_vector, start_time))
        timer_thread.start()
        data_thread.start()
    ...
    # Ensure size alignment for time_vector
    window_size = int(5 * sampling_rate)
    tolerance = 3
    match_found = False

    # Comparison using 5 random 5-second windows
    for i in range(5):
        ...
        # Check for common points, excluding values 279-283
        ...
        if np.sum(common_points) >= 4:
            ...
            # Turn on the LED by sending the signal to Arduino
            with serial.Serial('COM7', 9600, timeout=1) as ser:
                time.sleep(2)  # Short delay to allow the port to settle
